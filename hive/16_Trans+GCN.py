...
class Trans_Dataset(torch.utils.data.Dataset):
...
# 实例化LSTM数据集
Trans_train_dataset = Trans_Dataset("./all_data/split_data/train_pkl.pkl")
Trans_val_dataset   = Trans_Dataset("./all_data/split_data/val_pkl.pkl")
Trans_test_dataset  = Trans_Dataset("./all_data/split_data/test_pkl.pkl")


# 实例化LSTM数据集Dataloader
Trans_train_loader = DataLoader(Trans_train_dataset, batch_size=64, shuffle=False)
Trans_val_loader = DataLoader(Trans_val_dataset, batch_size=1, shuffle=False)
Trans_test_loader = DataLoader(Trans_test_dataset, batch_size=1, shuffle=False)
...
class FusionModel(nn.Module):
    def __init__(self):
        ...
        self.GCN_cell = GCN_model()  # [batch,128]
        # 融合层
...
if __name__ == '__main__':
    ...
    for epoch in range(200):
        ...
        train_loss_sum = 0
        # batch_size = 0
        for lstm_data, gcn_data in zip(Trans_train_loader, GCN_train_loader):
            # print(lstm_data)
        ...
        with torch.no_grad():
            ...
            val_loss_sum = 0
            for lstm_data, gcn_data in zip(Trans_val_loader, GCN_val_loader):
                # 获取LSTM数据
                lstm_inputs, lstm_targets, lstm_targets_mask, train_csv_names = lstm_data
                lstm_inputs, lstm_targets, lstm_targets_mask = lstm_inputs.to(device), lstm_targets.to(
                    device), lstm_targets_mask.to(device)
...
print(f"============ 模型测试开始 ============")
# 模型测试
test_model = torch.load("./model_save/Trans+GCN.pth")
# 模型验证(评估当前训练模型的性能)
test_model.eval()
...
sample_index = 0
with torch.no_grad():
    for lstm_data, gcn_data in zip(Trans_test_loader, GCN_test_loader):
        # print()
        sample_index += 1
        # print(sample_index)
