    ...
    for root, dirs, files in os.walk(cfg_path):
        for file in files:
            if file.endswith(".txt"):
                ...
                cfg_pattern = re.compile("^\$r\d+ = (?:staticinvoke |r\d+\.|\$r\d+\.)?<[\w\.\$\_\']+: org\.apache\.log4j\.Logger (?:\w+|access\$\d+\(\))>[()]{0,2};"
                                         "(?:(?!^Block\s\d+:)[\w\W])*?"
                                         "virtualinvoke \$r\d+\.<org\.apache\.log4j\.Logger: void (?:trace|debug|info|warn|error|fatal)\([\w\W\([\w\W]+?\);\n",
                                         re.MULTILINE)
                ...
                if bytecode_log_num == cfg_log_num:
                    ...
                    for log_index, cfg_log in enumerate(cfg_log_list):
                        ...
                        # 使用 np.vstack 将向量列表堆叠成 (n,768) 矩阵
                        ...
                        node_emedding_path = "./all_data/log4j_node"
                        ...
                        if empty_block_num != -1:
                            ...
                            # 保存edge_npy文件
                            train_edge_path = "./all_data/log4j_edge"
                    ...
                    train_csv_dir = "./all_data/log4j_csv"
                    ...
                    train_lable_dir = "./all_data/log4j_label"
