...
def delete_log_node(cfg_path, bytecode_txt_path):
    ...
    # 遍历文件夹中的 all_cfg 方法, 其中每个 all_cfg 文件都包含日志对应 train_csv 中的方法.
    for root, dirs, files in os.walk(cfg_path):
        for file in files:
            if file.endswith(".txt"):
                ...
                print("type_cfg_content:", type(cfg_content))
                # print("方法控制流图 cfg_content: ")
                # print(cfg_content)
                cfg_file.close()

                # 用于匹配 cfg_content 中日志语句的正则表达式
                cfg_pattern = re.compile(r"^\$r\d+ = (?:staticinvoke |r\d+\.|\$r\d+\.)?<[\w\.\$\_\']+: org\.slf4j\.Logger (?:\w+|access\$\d+\(\))>[\(\)]{0,2};"
                                         r"(?:(?!^Block\s\d+:)[\w\W])*?"
                                         r"interfaceinvoke \$r\d+\.<org\.slf4j\.Logger: void (?:trace|debug|info|warn|error|fatal)\([\w\W]+?\)>\([\w\W]+?\);\n"
                                         , re.MULTILINE)
                ...
                bytecode_pattern = re.compile(r"(?:L\d+\n)?"
                                              r"(?:GETSTATIC|INVOKESTATIC|GETFIELD) [\w/$().]+?\.(?:\w+ : |access\$\d+\s+\(\))Lorg/slf4j/Logger;"
                                              r"(?:(?!\nINVOKEINTERFACE org/slf4j/Logger\.(?:isTraceEnabled|isDebugEnabled|isInfoEnabled|isWarnEnabled|isErrorEnabled|isFatalEnabled) \(\)Z \(itf\))[\w\W])+?"
                                              r"(?:INVOKEINTERFACE|INVOKEVIRTUAL) org/slf4j/Logger\.(?:trace|debug|info|warn|error|fatal) [\w\/;[\$\(\)]+\s\(itf\)\n"
                                              r"(?:GOTO L\d+\n)?")

                # 找到bytecode中的日志,以列表的形式返回
                ...
                if bytecode_log_num == cfg_log_num:
                    ...
                    for log_index, cfg_log in enumerate(cfg_log_list):
                        ...
                        new_cfg_content = cfg_content.replace(cfg_log, "")
                        ...
                        block_num_list = []  # 记录方法 cfg 中节点的编号
                        edge_index = []  # 邻接矩阵
                        ...
                        # 使用 np.vstack 将向量列表堆叠成 (n,768) 矩阵, n 表示节点的个数
                        ...
                        # 保存 node_embedding 文件
                        node_emedding_path = "./all_data/slf4j_node"
                        ...
                        if empty_block_num != -1:
                            ...
                            # 保存edge_npy文件
                            train_edge_path = "./all_data/slf4j_edge"
                    ...
                    train_csv_dir = "./all_data/slf4j_csv"
                    ...
                    train_lable_dir = "./all_data/slf4j_label"
...
if __name__ == '__main__':
    cfg_path = "./all_data/slf4j_cfg"
    bytecode_txt_path = "/zookeeper/class-txt2"
