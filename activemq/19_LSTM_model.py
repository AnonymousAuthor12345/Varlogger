...
class myModel(nn.Module):
    def __init__(self, num_hiddens, num_layers, num_class):
        super().__init__()
        ...
        self.linear1 = nn.Linear(2 * num_hiddens, num_hiddens)
    ...
    def forward(self, x):
        ...
        # print("   lstm hidden.shape:", h.shape)

        # cat层
        cat_hidden = torch.cat((h[-2], h[-1]), -1)
        # print("   cat hidden.shape:", cat_hidden.shape)

        # 线性层1
        cat_hidden = F.relu(self.linear1(cat_hidden))
        # print("   linear1 output.shape:", cat_hidden.shape)

        # 分类器
        cat_hidden = self.classifier(cat_hidden)
        # print("   classifier output.shape:", cat_hidden.shape)  # [128, 8]
        return cat_hidden


# 实例化模型
model1 = myModel(num_hiddens=128, num_layers=2, num_class=10)
...
if __name__ == '__main__':
    ...
    # 模型测试
    print(f"============ 模型测试开始 ============")
    test_model = torch.load("./model_save/BiLstm_model.pth")
    # 模型验证(评估当前训练模型的性能)
    test_model.eval()
    ...
    with torch.no_grad():
        for batch_index, data in enumerate(test_loader):
            ...
            # all_label 列表
            label_list = label_csv['all_label'].tolist()
