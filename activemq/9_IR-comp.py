...
def IR_process(train_path, test_path):
    ...
    train_token_list = []
    # 读取对应名字的 csv 数据
    for csv_name in train_name_list:
        # print("=============="+csv_name+"==============")
        train_csv_dir = "./all_data/train_csv"
        train_csv_path = os.path.join(train_csv_dir, csv_name)
        df1 = pd.read_csv(train_csv_path)
        # print("df1:")
        # print(df1['methodContent'].tolist())
        method_content_list = df1["methodContent"].tolist()
        # 把每行数据拼成一个长字符串
        bytecode_str = '\n'.join(method_content_list)
        bytecode_tokens = re.split(r'(\W+)', bytecode_str)  # 按照非字母数字下划线进行分割单词
        bytecode_tokens = " ".join(bytecode_tokens)
        tuple1 = (csv_name, bytecode_tokens)
    ...
    for csv_name in test_name_list:
        # print("==============" + csv_name + "==============")
        train_csv_dir = "./all_data/train_csv"
        train_csv_path = os.path.join(train_csv_dir, csv_name)
        df2 = pd.read_csv(train_csv_path)
        # print("df2:")
        # print(df1['methodContent'].tolist())
        method_content_list = df2["methodContent"].tolist()
        # 把每行数据拼成一个长字符串
        bytecode_str = '\n'.join(method_content_list)
        bytecode_tokens = re.split(r'(\W+)', bytecode_str)  # 按照非字母数字下划线进行分割单词
        # print("bytecode_tokens:", bytecode_tokens)
        bytecode_tokens = " ".join(bytecode_tokens)
        tuple2 = (csv_name, bytecode_tokens)
    ...
    print("123:", X_train.shape)
    print("456:", X_test.shape)
    # 计算余弦相似度
    ...
    for test_index, test_name in enumerate(test_name_list):
        ...
        pred_path = os.path.join(train_label_dir, pred_csv_name)
        df4 = pd.read_csv(pred_path)
        pred_label_list = df4["label"].tolist()
        pred_var_name_list = df4["var_name"]

        pred_list = []
        for idx1, label_item in enumerate(pred_label_list):
            if label_item == 1:
                pred_list.append(pred_var_name_list[idx1])
        pred_list.sort()
