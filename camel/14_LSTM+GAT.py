...
class LSTM_model(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # 线性层1
        cat_hidden = F.relu(self.linear1(cat_hidden))
...
class FusionModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.LSTM_cell = LSTM_model()  # [batch,128]
        self.GCN_cell = GAT_model()  # [batch,128]
...
if __name__ == '__main__':
    ...
    for epoch in range(200):
        ...
        for lstm_data, gcn_data in zip(LSTM_train_loader, GCN_train_loader):
            ...
            lstm_inputs, lstm_targets, lstm_targets_mask = lstm_inputs.to(device), lstm_targets.to(device), lstm_targets_mask.to(device)
    ...
    print(f"============ 模型测试开始 ============")
    # 模型测试
    test_model = torch.load("./model_save/LSTM+GAT.pth")
    # 模型验证(评估当前训练模型的性能)
    test_model.eval()
