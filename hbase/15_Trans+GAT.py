...
class LSTM_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (train_data['bytecode_tensor'], train_data['label_tensor'],
                train_data['label_mask_tensor'], train_data['train_name'])


# 实例化LSTM数据集
LSTM_train_dataset = LSTM_Dataset("./all_data/split_data/train_pkl.pkl")
LSTM_val_dataset   = LSTM_Dataset("./all_data/split_data/val_pkl.pkl")
LSTM_test_dataset  = LSTM_Dataset("./all_data/split_data/test_pkl.pkl")

# 获取LSTM数据集长度
LSTM_train_dataset_size = len(LSTM_train_dataset)
LSTM_val_dataset_size = len(LSTM_val_dataset)
LSTM_test_dataset_size = len(LSTM_test_dataset)

# 实例化LSTM数据集Dataloader
LSTM_train_loader = DataLoader(LSTM_train_dataset, batch_size=64, shuffle=False)
LSTM_val_loader = DataLoader(LSTM_val_dataset, batch_size=1, shuffle=False)
LSTM_test_loader = DataLoader(LSTM_test_dataset, batch_size=1, shuffle=False)

class Trans_model(nn.Module):
    def __init__(self, d_model=768, nhead=8, num_layers=2, num_classes=10):
        super().__init__()
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dim_feedforward=768,
            dropout=0.3,
            batch_first=True  # 允许输入 [batch, seq, dim]
        )
...
# # GCN_DataLoader实例化
GCN_train_loader = PYG_DataLoader(GCN_train_dataset, batch_size=64, shuffle=False)
GCN_val_loader = PYG_DataLoader(GCN_val_dataset, batch_size=1, shuffle=False)
GCN_test_loader = PYG_DataLoader(GCN_test_dataset, batch_size=1, shuffle=False)
...
class FusionModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.Trans_cell = Trans_model()  # [batch,128]
        self.GAT_cell = GAT_model()     # [batch,128]
        # 融合层
        self.fusion_linear = nn.Linear(256, 128)  # 融合LSTM和GCN拼接的输出向量
        self.classifier = nn.Linear(128, 10)  # 最终分类类别数预测(20分类)

    def forward(self, lstm_input1, gcn_input1, gcn_edge_index1, gcn_batch1):
        # LSTM
        lstm_output = self.Trans_cell(lstm_input1)

        # GCN
        gcn_output = self.GAT_cell(gcn_input1, gcn_edge_index1, gcn_batch1)
...
if __name__ == '__main__':
    ...
    for epoch in range(200):
        ...
        train_loss_sum = 0
        # batch_size = 0
        for lstm_data, gcn_data in zip(LSTM_train_loader, GCN_train_loader):
            # print(lstm_data)
            lstm_inputs, lstm_targets, lstm_targets_mask, train_csv_names = lstm_data
            lstm_inputs, lstm_targets, lstm_targets_mask = lstm_inputs.to(device), lstm_targets.to(
                device), lstm_targets_mask.to(device)
            # csv_name = train_csv_names[0]

            # 获取 GCN 数据
            gcn_inputs = gcn_data.x.to(device)
            gcn_edge_index = gcn_data.edge_index.to(device)
            gcn_batch = gcn_data.batch.to(device)

            # 梯度清零
            optimizer.zero_grad()

            # forward
            outputs = fusion_model(lstm_inputs, gcn_inputs, gcn_edge_index, gcn_batch)
            loss = criterion(outputs, lstm_targets, lstm_targets_mask)
        ...
        with torch.no_grad():
            ...
            val_loss_sum = 0
            for lstm_data, gcn_data in zip(LSTM_val_loader, GCN_val_loader):
                # 获取LSTM数据
                lstm_inputs, lstm_targets, lstm_targets_mask, train_csv_names = lstm_data
                lstm_inputs, lstm_targets, lstm_targets_mask = lstm_inputs.to(device), lstm_targets.to(
                    device), lstm_targets_mask.to(device)
                ...
                mask_list = lstm_targets_mask.squeeze().tolist()
            ...
            val_average_acc_list.append(average_acc)
            ...
            # 计算当前epoch下,模型的 map
            val_map = calculate_map(recommendations=recommendations_list,
                                    true_logged_variables_list=true_logged_variables_list)
    ...
    print(f"============ 模型测试开始 ============")
    # 模型测试
    test_model = torch.load("./model_save/Trans+GAT.pth")
    # 模型验证(评估当前训练模型的性能)
    test_model.eval()
    ...
    sample_index = 0
    with torch.no_grad():
        for lstm_data, gcn_data in zip(LSTM_test_loader, GCN_test_loader):
            # print()
            sample_index += 1
            # print(sample_index)

            # 获取LSTM数据
            lstm_inputs, targets, targets_mask, train_csv_names = lstm_data
            lstm_inputs, targets, targets_mask = lstm_inputs.to(device), targets.to(device), targets_mask.to(device)
            csv_name = train_csv_names[0].split(".txt")[0]
            ...
            # 模型测试
            outputs = test_model(lstm_inputs, gcn_inputs, gcn_edge_index, gcn_batch)
    ...
    print("test_acc:", test_acc_result)
